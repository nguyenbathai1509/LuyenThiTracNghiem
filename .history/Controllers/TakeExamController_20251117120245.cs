...
namespace LuyenThiTracNghiem.Controllers
{
    public class TakeExamController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Submit([FromBody] SubmitExamModel model)
        {
            ...
            attempt.DurationSeconds = (int)(System.DateTime.UtcNow - attempt.StartedAt).TotalSeconds;

            await _context.SaveChangesAsync();

            // trả về JSON kèm url kết quả
            return Json(new { redirectUrl = Url.Action("Result", new { attemptId = attempt.AttemptId }) });
        }
    }
}
