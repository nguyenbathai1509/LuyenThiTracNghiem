...
namespace LuyenThiTracNghiem.Controllers
{
    public class TakeExamController : Controller
    {
        ...
        {
            var attempt = _context.ExamAttempts.FirstOrDefault(x => x.AttemptId == attemptId);
            if (attempt == null) return NotFound();

            if (attempt.IsCompleted)
            {
                // Nếu đã nộp rồi → chuyển qua xem kết quả
            ...
            var questionIds = qInExam.Select(x => x.QuestionId).ToList();
            ...
            var userAnswers = _context.AttemptAnswers
            ...
            foreach (var q in qInExam)
            {
                var ua = userAnswers.FirstOrDefault(x => x.QuestionId == q.QuestionId);
                var correctAns = correctAnswers.FirstOrDefault(a => a.QuestionId == q.QuestionId);

                if (ua != null)
                {
                    if (ua.AnswerId == correctAns.AnswerId)
                    {
                        ua.IsCorrect = true;
                        correct++;
                    }
            ...
            // Tính điểm (VD: 10 điểm, chia đều)
            double score = Math.Round((double)correct / qInExam.Count * 10, 2);
