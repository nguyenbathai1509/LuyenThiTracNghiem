...
namespace LuyenThiTracNghiem.Controllers
{
    public class TakeExamController : Controller
    {
        ...
        {
            if (attemptId <= 0) return BadRequest();
            ...
            // Lấy tất cả câu hỏi của attempt
            var questionIds = _context.QuestionInExams
            ...
            {
                var value = form[$"q_{qid}"]; // name="q_QuestionId" trên form
                int? selectedAnswerId = string.IsNullOrEmpty(value) ? null : int.Parse(value);

                var correctAnswer = await _context.Answers
                    .Where(a => a.QuestionId == qid && a.IsCorrect)
                    .FirstOrDefaultAsync();
            ...
            // submit form bình thường -> redirect trực tiếp
        ...
        public async Task<IActionResult> Result(int attemptId)
{
    ...
    var questions = await _context.Questions
        .Where(q => q.ExamId == attempt.ExamId)
        .Select(q => new ExamQuestionResultViewModel
        {
            QuestionId = q.QuestionId,
            QuestionText = q.QuestionText,
    ...
    var vm = new ExamResultViewModel
    {
        ...
        IncorrectCount = attempt.WrongCount,
