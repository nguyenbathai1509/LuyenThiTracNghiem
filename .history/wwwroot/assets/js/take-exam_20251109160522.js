    ...
    document.querySelectorAll('.question-link').forEach(a => {
        a.addEventListener('click', function (ev) {
            ...
            if (questionsList && getComputedStyle(questionsList).overflowY !== 'visible') {
                ...
                const containerTop = questionsList.getBoundingClientRect().top;
                const targetTop = target.getBoundingClientRect().top;
                const offset = targetTop - containerTop + questionsList.scrollTop;
                questionsList.scrollTo({ top: offset - 8, behavior: 'smooth' }); // -8 for small padding
            } else {
